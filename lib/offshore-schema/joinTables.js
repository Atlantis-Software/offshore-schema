...
JoinTables.prototype.markCustomJoinTables = function(collection) {
  ...
  for (var attribute in attributes) {
    ...
    var linkedCollectionName = attributes[attribute].through.toLowerCase();
    var via = attributes[attribute].via;
    var linkedCollection = this.collections[linkedCollectionName];
    if (!linkedCollection) {
      throw new Error('Can\'t find through table ' + linkedCollectionName + ' from ' + collection + '.' + attribute);
    }
    linkedCollection.throughTable = linkedCollection.throughTable || {};
    var throughPath = collection + '.' + attribute;
    var linkedAttrs = linkedCollection.attributes;
    linkedCollection.throughTable[throughPath] = _.find(_.keys(linkedAttrs), function(attr){
      return linkedAttrs[attr].references === attributes[attribute].collection.toLowerCase();
    });

    // Build up proper reference on the attribute
    attributes[attribute].collection = linkedCollectionName;
    attributes[attribute].references = linkedCollectionName;

    // Find Reference Key
    var reference = this.findReference(collection, linkedCollectionName, via);
    attributes[attribute].on = linkedAttrs[reference].columnName || reference;
