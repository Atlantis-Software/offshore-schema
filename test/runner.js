  ...
  if (failures) {
    ...
    download('Atlantis-Software/offshore', path.join(__dirname, '..', 'offshoreTest'), function (err) {
      ...
      offshoreInstall.on('exit', function(code) {
        rimraf(path.join(__dirname, '..', 'offshoreTest', 'node_modules', 'offshore-schema'), function(err) {
          if (err) {
            throw err;
          }
          fs.symlink(path.join(__dirname, '..'), path.join(__dirname, '..', 'offshoreTest', 'node_modules', 'offshore-schema'), function(err) {
            if (err) {
              throw err;
            }
